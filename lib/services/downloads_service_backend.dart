...
class DownloadsSyncService {
  ...
  Future<void> _prefillPlaylistAlbumChildren(
    Iterable<DownloadStub> playlistTracks,
  ) async {
    const albumBatchSize = 10;
    final albumIds = playlistTracks
        .where((track) => track.type == DownloadItemType.track)
        .map((track) => track.baseItem?.albumId)
        .nonNulls
        .where((albumId) => !_childCache.containsKey(albumId.raw))
        .toSet()
        .toList();
    ...
    benchmark?.maxMetricBuffered(
      ...
      albumIds.length.clamp(0, albumBatchSize),
    );

    // Reserve every album before the first request so another sync worker
    // cannot issue a per-album request while the playlist-wide prefetch is
    // still working through later chunks.
    final reservations = <BaseItemId, Completer<List<String>>>{};
    ...
    for (final albumChunk in albumIds.slices(albumBatchSize)) {
      ...
      try {
        ...
        if (requestStopwatch != null) {
          ...
          final activeBenchmark = PerformanceBenchmarkService.instance;
        ...
        var coveredAlbums = 0;
        for (final albumId in albumChunk) {
          final reservation = reservations[albumId]!;
        ...
        _syncLogger.fine(
          "Playlist album child prefetch failed; using normal album requests on retry: $e",
        );
      }
    }
  }
