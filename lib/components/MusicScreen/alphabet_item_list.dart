...
class _AlphabetListState extends ConsumerState<AlphabetList> {
  ...
  void initState() {
    ...
    _benchmarkTapSubscription =
        PerformanceBenchmarkService.instance.alphabetTapCommands.listen(
      _handleBenchmarkTap,
    );
    super.initState();
  }
  ...
  void _handleBenchmarkTap(PerformanceBenchmarkJumpCommand command) {
    if (!mounted ||
        widget.benchmarkContentType == null ||
        widget.benchmarkContentType != command.contentType) {
      return;
    }
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (index < 0 || _letterHeight <= 0) {
        command.completeError(
          StateError("Benchmark alphabet tap target is unavailable"),
          StackTrace.current,
        );
        return;
      }
